...
public class LocaleLinearLayoutManager extends LinearLayoutManager {
    ...
    public boolean performAccessibilityActionForItem(RecyclerView.Recycler recycler,
        ...
        if(R.id.action_drag_move_up == action){
            if (position > 0) {
                mAdapter.onItemMove(position, position - 1);
                result = true;
            }
        }else if(R.id.action_drag_move_down == action){
            if (position + 1 < itemCount) {
                mAdapter.onItemMove(position, position + 1);
                result = true;
            }
        }else if(R.id.action_drag_move_top == action){
            if (position != 0) {
                mAdapter.onItemMove(position, 0);
                result = true;
            }
        }else if(R.id.action_drag_move_bottom == action){
            if (position != itemCount - 1) {
                mAdapter.onItemMove(position, itemCount - 1);
                result = true;
            }
        }else if(R.id.action_drag_remove == action){
            if (itemCount > 1) {
