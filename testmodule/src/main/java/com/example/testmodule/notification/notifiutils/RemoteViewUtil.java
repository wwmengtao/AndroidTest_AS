...
public class RemoteViewUtil {
    ...
    //获取自定义通知管理视图
    public static RemoteViews getManagerNotifyRemoteViews(Context mContext) {
        RemoteViews mRemoteViews = new RemoteViews(mContext.getPackageName(), R.layout.notification_custom_manage_notifi);
        mRemoteViews.setImageViewResource(R.id.iv_title, R.drawable.moto);
        mRemoteViews.setTextViewText(R.id.tv_title, mContext.getString(R.string.notifi_office_title));
        mRemoteViews.setTextViewText(R.id.tv_subtext1, mContext.getString(R.string.notifi_office_des));
        mRemoteViews.setTextViewText(R.id.tv_subtext2, mContext.getString(R.string.app_nofity_whitelist));
        //
        return mRemoteViews;
    }
}
